...
from team_finder.constants import MESSAGES, STATE, LIMITS
...
def toggle_usership(request, project_id):
    ...
    return JsonResponse({"status": "ok", "joined": not already_in})
...
def skills_autocomplete(request):
    ...
    if term:
        found = Skill.objects.filter(name__istartswith=term)[: LIMITS["autocomplete"]]
    else:
        found = Skill.objects.none()

    return JsonResponse([{"id": s.id, "name": s.name} for s in found], safe=False)
...
def attach_skill(request, project_id):
    ...
    if project is None:
        return JsonResponse(
            {"error": MESSAGES["project_missing"]}, status=HTTPStatus.NOT_FOUND
        )

    if project.owner != request.user:
        return JsonResponse(
            {"error": MESSAGES["access_denied"]}, status=HTTPStatus.FORBIDDEN
        )
    ...
    try:
        ...
        return JsonResponse(
            {"error": MESSAGES["bad_json"]}, status=HTTPStatus.BAD_REQUEST
        )
    ...
    if sid:
        ...
        if skill is None:
            return JsonResponse(
                {"error": MESSAGES["skill_missing"]}, status=HTTPStatus.NOT_FOUND
            )
    elif skill_title:
        skill, created_at = Skill.objects.get_or_create(name=skill_title.strip())
    else:
        return JsonResponse(
            {"error": MESSAGES["skill_data_required"]}, status=HTTPStatus.BAD_REQUEST
        )
...
def detach_skill(request, project_id, skill_id):
    ...
    return JsonResponse(
        {"error": MESSAGES["skill_not_present"]}, status=HTTPStatus.BAD_REQUEST
    )
